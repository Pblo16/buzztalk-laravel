<div>
    <div x-data="{ 
        ...
        x-effect="resetContent()"
        class="fixed inset-0 z-[70] overflow-y-auto flex items-center justify-center"
        style="display: none;">
        <div class="min-h-screen w-full px-4 text-center flex items-center justify-center">
            ...
            <div class="inline-block w-full max-w-4xl bg-white dark:bg-gray-800 rounded-lg text-left shadow-xl transform transition-all align-middle">
                @if($post)
                <div class="flex max-h-[85vh]">
                    <div class="w-2/3 bg-black flex items-center justify-center">
                        ...
                        @else
                        <img src="{{ $post->media_url }}" alt="Post" class="w-full h-full object-contain" :key="'{{ $post->id }}'">
                        @endif
                    </div>
